...
class Config:
    ...
    splits: List[str] = field(default_factory=lambda: ["train_sft", "train_gen"])  # Splits à utiliser
...
class ConversationData(pydantic.BaseModel):
    ...
    def get_message_counts(self) -> Dict[str, int]:
        ...
        return {
            ...
            "user_messages_count": len([m for m in self.messages if m["role"] == "user"]),
            "assistant_messages_count": len([m for m in self.messages if m["role"] == "assistant"])
        }
...
class DatasetProcessor:
    ...
    def process_embeddings(self, texts: List[str], entries: List[ConversationData], model):
        ...
        try:
            ...
            with init_model(self.device) as model:
                for start_idx in range(0, len(dataset_train), self.config.chunk_size):
                    end_idx = min(start_idx + self.config.chunk_size, len(dataset_train))
                    batch = dataset_train[start_idx:end_idx]
                    
                    logger.info(f"⚙️ Processing batch {start_idx//self.config.chunk_size + 1}")
